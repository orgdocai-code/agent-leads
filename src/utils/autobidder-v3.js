...
async function searchBountyIssues() {
  ...
  try {
    // Search for issues with bounty label
    const queries = [
...
async function createSolutionPR(issue) {
  ...
  try {
    ...
    const prRes = await githubAxios.post(`/repos/${owner}/${repo}/pulls`, {
      ...
      body: `## Solution by AgentLeads 🤖\n\n${issue.body ? `Original Issue: ${issue.body.substring(0, 500)}` : ''}\n\n---\n*This PR was automatically generated by [AgentLeads](https://agent-leads.vercel.app)*`,
      head: branchName,
      base: defaultBranch
    });
...
function parseSolutionFiles(solution) {
  ...
  // Fallback: if no files parsed, create a single solution.md
  if (files.length === 0) {
...
async function checkGitHubBounties() {
  ...
  if (result.issues.length > lastBountyCount && lastBountyCount > 0) {
    ...
    // Auto-create PR for the newest issue
...
app.post('/autobid/github/start', (req, res) => {
  ...
  checkGitHubBounties();
  githubInterval = setInterval(checkGitHubBounties, 30000); // Check every 30s
